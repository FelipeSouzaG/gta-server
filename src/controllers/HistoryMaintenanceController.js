...
class HistoryMaintenanceController {
  ...
  static generateQrCode = async (req, res, next) => {
    try {
      const { level } = req.userData;
      const { envId } = req.params;
      const envEquipment = await Environment.findById(envId);
      if (!envEquipment) {
        return next(
          new NotFound({
            title: 'ID do Ambiente',
            msg: 'Id do Ambiente não encontrado no banco de dados',
          })
        );
      }
      ...
      }?envId=${envId}&equipmentName=${encodeURIComponent(
        envEquipment.equipmentNumber
      )}`;

      const qrCode = await qrcode.toDataURL(historyURL);

      res.status(200).json({
        title: `QRCODE Link ${envEquipment.equipmentNumber}`,
        msg: qrCode,
        number: envEquipment.equipmentNumber,
        status: 200,
        level: level,
      });
    } catch (error) {
