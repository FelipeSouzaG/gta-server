...
import { Request, Client, Address, Environment } from '../models/Model.js';
import { RequestError, ErrorBase, NotFound } from '../erros/Errors.js';
...
class RequestController {
  ...
  static getClientRequests = async (req, res, next) => {
    try {
      ...
      const requests = await Request.find({
        clientId: client._id,
        /*requestStatus: { $ne: 'Finalizado' }, ver se todos ou excluir Finalizados*/
      })
        .populate(
          'addressId',
  ...
  static updateRequest = async (req, res, next) => {
    ...
    try {
      ...
      let returnRes = {};
      if (updatedRequest.requestStatus === 'Visita Técnica Programada') {
  ...
  static getAddressRequests = async (req, res, next) => {
    try {
      const { level } = req.userData;
      ...
      if (requests.length === 0) {
        return next(
          new RequestError({
            title: 'Não há Requisições!',
            msg: 'Não há Requisições para esse endereço no banco de dados',
          })
        );
      }
  ...
  static getAddressEnvironmentId = async (req, res, next) => {
    try {
      const { level } = req.userData;
      const { envId } = req.params;

      const environment = await Environment.findOne({ envId });

      if (!environment) {
        return next(
          new NotFound({
            title: 'Não Encontrado!',
            msg: 'Ambiente não encontrado no banco de dados.',
          })
        );
      }
      ...
      if (requests.length === 0) {
        return next(
          new RequestError({
            title: 'Não há Requisições!',
            msg: 'Não há Requisições para esse ambiente no banco de dados',
          })
        );
      }
